...
const expectSourceExample = (
  ...
  const link = [...element.querySelectorAll<HTMLAnchorElement>('.qdb-source-example')].at(index);
...
describe('EditEntityDialog', () => {
  ...
  it('validates identity fields and returns an updated team relationship', async () => {
    ...
    const leagueSelect = await loader.getHarness(
      MatSelectHarness.with({ selector: '.qdb-league-select' }),
    );
    ...
    expect(element.querySelector('.qdb-source-note h3')?.textContent).toBe(
      'How Soccerbot builds source links',
    );
    expect(element.querySelectorAll('.qdb-source-example')).toHaveLength(1);
    expectSourceExample(
      element,
      0,
      'Team page',
    ...
    expect(element.querySelector('.qdb-source-note-detail')?.textContent).toContain(
  ...
  it('locks Soccerway, hides its season, validates path IDs, and explains regenerated URLs', async () => {
    ...
    expect(element.querySelectorAll('.qdb-source-example')).toHaveLength(2);
    expectSourceExample(
      element,
      0,
      'Team page',
