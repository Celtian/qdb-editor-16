...
const buildAppIcon = async (): Promise<void> => {
  ...
  const sourcePath = join(root, 'resources', 'icons', 'qdb-editor-16-source.png');
  ...
  await Promise.all([
    writeOutput(join(root, 'resources', 'icons', 'qdb-editor-16.png'), appPng),
    writeOutput(join(root, 'resources', 'icons', 'qdb-editor-16.ico'), appIco),
    writeOutput(join(root, 'projects', 'electron', 'public', 'qdb-editor-16-icon.png'), appPng),
